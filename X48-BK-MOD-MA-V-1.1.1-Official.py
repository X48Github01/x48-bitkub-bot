...
FASTEMAi = dbconf.get('SETTING', 'FAST_MA_VALUE*').split(",")
MIDTYPEi = dbconf.get('SETTING', 'MID_TYPE*').split(",")
MIDEMAi = dbconf.get('SETTING', 'MID_MA_VALUE*').split(",")
SLOWTYPEi = dbconf.get('SETTING', 'SLOW_TYPE*').split(",")
SLOWEMAi = dbconf.get('SETTING', 'SLOW_MA_VALUE*').split(",")
...
balance_notify = dbconf.get("OPTION","TIME_BALANCE_NOTIFY")
double_check = dbconf.get("OPTION","DOUBLE_CHECK")

# ! ========================= Balance Check & Notify ====================
if balance_notify == '15m':
	balance_notify_value = 15
	balance_notify_tag = 15
if balance_notify == '30m':
	balance_notify_value = 30
	balance_notify_tag = 30
if balance_notify == '1h':
	balance_notify_value = 60
	balance_notify_tag = 1
if balance_notify == '4h':
	balance_notify_value = 240
	balance_notify_tag = 4
if balance_notify == '1d':
	balance_notify_value = 1440
	balance_notify_tag = 1
if balance_notify_value < 60:
	balance_notify_type = 'นาที'
if balance_notify_value > 59:
	balance_notify_type = 'ชั่วโมง'
if balance_notify == '1d':
	balance_notify_type = 'วัน'
# ! ============== Loop TimeFrame Type ===================
if  looptimeframe == '1s':
...
def trade(symbol, df, EMAfast, EMAmid, EMAslow, order, TF, FASTEMA, MIDEMA, SLOWEMA, FASTTYPE, MIDTYPE, SLOWTYPE, TP_Mode, SL_Mode, TP_Percent, SL_Percent, PNL_Mode, TP_PNL, SL_PNL, pnl_cal_text, pnl_percent_change_text):
	wall = bitkub.wallet()
	res = 'result'
	t = 'THB'
...
def balance_check():
	...
	mes=f'\n⏰⏰แจ้งเตือนทุก {balance_notify_tag} {balance_notify_type} ค่ะนายท่าน\n\n{version}\n\n⭐Status : ระบบทำงานปกติค่ะ\n\n💰ยอดเงินคงเหลือ : {amtwall} บาท\n\nAccount : {Account_Name}'
...
def main():
	...
	for i in range(len(Symbol)):
		try:
			...
			#print(f'{symbol}')
			if mode_trade == 'ON':
				trade(symbol, df, EMAfast, EMAmid, EMAslow, order, TF, FASTEMA, MIDEMA, SLOWEMA, FASTTYPE, MIDTYPE, SLOWTYPE, TP_Mode, SL_Mode, TP_Percent, SL_Percent, PNL_Mode, TP_PNL, SL_PNL, pnl_cal_text, pnl_percent_change_text)
		except Exception as Error:
...
if __name__ == "__main__":
	while True:
		...
		local_time = time.ctime(seconds)
		# ! Thread Setting
		main_thread1 = threading.Thread(target=main, name='main_thread1')
		double_thread1 = threading.Thread(target=main, name='double_thread1')
		main_wallet_check = threading.Thread(target=balance_check, name='main_wallet_check')
		logo_thread1 = threading.Thread(target=text_logo, name='logo_thread')
		logo_double_thread1 = threading.Thread(target=text_logo, name='logo_double_thread')
		...
		if (str(local_time[14:-5]) == '00:00' or str(local_time[14:-5]) == '15:00' or str(local_time[14:-5]) == '30:00' or str(local_time[14:-5]) == '45:00') :
			# ! ======== Every 15 Minute =========
			if looptimeframe == '15m' or balance_notify == '15m':
				if looptimeframe == '15m':
					time.sleep(1)
					main_thread1.run()
					logo_thread1.run()
		...
		if (str(local_time[14:-5]) == '00:00' or str(local_time[14:-5]) == '30:00') :
			...
			if (str(local_time[14:-5]) == '00:00') :
				...
				if looptimeframe == '1h' or balance_notify == '1h':
					if looptimeframe == '1h':
						time.sleep(1)
						main_thread1.run()
						logo_thread1.run()
		...
		if (str(local_time[11:-5]) == '00:00:00' or str(local_time[11:-5]) == '04:00:00' or str(local_time[11:-5]) == '08:00:00' or str(local_time[11:-5]) == '12:00:00' or str(local_time[11:-5]) == '16:00:00' or str(local_time[11:-5]) == '20:00:00') :
			...
			# else screen will be cleared for windows
				...
				os.system('cls')
				text_logo()
			if looptimeframe == '4h' or balance_notify == '4h':
				if looptimeframe == '4h':
					time.sleep(1)
					main_thread1.run()
					logo_thread1.run()
		...
		if (str(local_time[11:-5]) == '00:00:00') :
			if looptimeframe == '1d' or balance_notify == '1d':
				if looptimeframe == '1d':
					time.sleep(1)
					main_thread1.run()
					logo_thread1.run()
